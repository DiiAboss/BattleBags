...
varying vec2 v_vTexcoord; // Alternative texture coordinate
// In Game Maker Studio, the auto-declared texture is typically accessed differently
float t;
...
float pn( in vec3 x )
{
    ...
    // For Game Maker Studio, use texture2D with the default texture
    vec2 rg = texture2D(gm_BaseTexture, (uv + 0.5)/256.0).xy;
...
float map(vec3 p)
{
    ...
    float cyl = 0.;vec2 vecOld = vec2(0.);
    for (float i=0.;i<6.;i++)
    {
        float x = 1. * i;
        float y = .88 + 0.0102*i;
...
{
    vec2 g = fragCoord.xy;
    vec4 fragColor = vec4(0,0.15,0.32,1);
    t = iGlobalTime*2.5;
    ...
    vec2 q = g/si;
    fragColor.rgb *= 0.5 + 0.5*pow(abs( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y)), 0.25 );
