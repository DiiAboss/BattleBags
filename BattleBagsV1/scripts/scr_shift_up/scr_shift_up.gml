        ...
        if (i == parent_x && j == parent_y) {
            ...
            if (can_move) {
                for (var k = 0; k < ds_list_size(parts_to_move); k++) {
                    ...
                    // ✅ Move block to new position
                    grid[new_x, new_y_pos] = grid[old_x, old_y];
                    
                    grid[old_x, old_y] = grid[old_x, old_y + 1]; // This actually worked (I used to create an empty block here...
                    
                    grid[new_x, new_y_pos].big_parent = [parent_x, new_y]; // ✅ Update parent reference
                }
    
                // ✅ Ensure Mega Block remains properly referenced
                grid[parent_x, new_y].mega_width = big_block_width;
                grid[parent_x, new_y].mega_height = big_block_height;
            }
