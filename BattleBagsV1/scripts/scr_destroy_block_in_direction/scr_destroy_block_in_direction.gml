function destroy_blocks_in_direction_from_point(player, start_x, start_y, _dir_x, _dir_y, steps = 3) {
	...
	while (_steps < steps && i >= 0 && i < width && j >= 1 && j < height) {
	    ...
	    points_awarded = calculate_match_points(player, blocks_destroyed);

	    // ✅ Mark block for delayed destruction
	    var pop_info =  create_pop_info(player, block, i, j, start_x, start_y, blocks_destroyed, points_awarded);
            ...
            for (var _i = 0; _i < self.width; _i++) {
                for (var _j = 0; _j < bottom_row; _j++) {
                    ...
                    if (other_block.group_id == group_id) {
                        ...
                        player.grid[_i, _j] = create_block(player, block.type);						
                        ...
                        player.grid[_i, _j].popping   = true;  // Start popping process
                        player.grid[_i, _j].pop_timer = dist * 5;
                        var _pitch = clamp(0.75 + (0.2 * player.combo), 0.5, 5);
                        if !(player.game_over_state)
                        {
    ...
    player.total_points += points_awarded;
	...
	var bow_obj = instance_create_depth(player.board_x_offset + start_x * player.gem_size + 32, (start_y * player.gem_size) + player.global_y_offset + 32, depth-99, obj_bow);
