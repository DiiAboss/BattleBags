...
for (var i = 0; i < array_length(menu_options); i++) {
    ...
    if (point_in_rectangle(mouse_x_pos, mouse_y_pos, menu_x - 300, menu_y - 20, menu_x + 300, menu_y + 20)) {
        selected_option = i;
...
if (block_spawn_timer >= 2) { // Every 2 frames
    block_spawn_timer = 0;
    
    // Try to spawn blocks based on intensity
    repeat(block_intensity) {
        create_falling_block(2, 4, 0.7, 1.3, 0.1, 100);
    }
}